...
def featureExtraction(low_res_input,level):
    ...
    with tf.name_scope("input"+str(level)):
        """
            input layer
        """
        conv_input = layer.conv2d(low_res_input, filters, filters, channel, level, name='conv_input')
    ...
    with tf.name_scope("deep_cnn"+str(level)):
        ...
        for i in range(depth):
            conv_cnn = layer.conv2d(last_lrelu, filters, filters, channel, level, isBias=False,
...
def imageReconstruction(low_res_input, conv_up,level):
    ...
    with tf.name_scope("Reconstruction"+str(level)):
        ...
        """
        deconv_image = layer.deconv2d(low_res_input, bilinear_upsample_weights(filters_tranpose, channel), [2, 2], level,
                                      name="deconv_image")
